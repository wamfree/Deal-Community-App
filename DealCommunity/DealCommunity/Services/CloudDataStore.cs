...
{
    public class CloudDataStore : IDataStore<Product>
    {
        HttpClient client;
        ...
        public async Task<IEnumerable<Product>> GetItemsAsync(bool forceRefresh = false)
        {
            ...
            {
                var json = await client.GetStringAsync($"products.json");
                items = await Task.Run(() => JsonConvert.DeserializeObject<IEnumerable<Product>>(json));
            }
